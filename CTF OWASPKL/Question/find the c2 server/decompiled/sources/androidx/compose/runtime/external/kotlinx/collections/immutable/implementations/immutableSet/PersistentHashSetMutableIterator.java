...
public final class PersistentHashSetMutableIterator<E> extends PersistentHashSetIterator<E> implements Iterator<E>, KMutableIterator {
    ...
    private final void resetPath(int hashCode, TrieNode<?> trieNode, E e, int pathIndex) {
        if (isCollision(trieNode)) {
            int index = ArraysKt.indexOf((E[]) trieNode.getBuffer(), e);
            CommonFunctionsKt.m2259assert(index != -1);
            getPath().get(pathIndex).reset(trieNode.getBuffer(), index);
            setPathLastIndex(pathIndex);
            return;
        }
        int position = 1 << TrieNodeKt.indexSegment(hashCode, pathIndex * 5);
        int index2 = trieNode.indexOfCellAt$runtime_release(position);
        getPath().get(pathIndex).reset(trieNode.getBuffer(), index2);
        Object cell = trieNode.getBuffer()[index2];
        if (cell instanceof TrieNode) {
            resetPath(hashCode, (TrieNode) cell, e, pathIndex + 1);
        } else {
            setPathLastIndex(pathIndex);
        }
    }
