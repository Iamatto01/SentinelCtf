...
public final class IdentityArrayMap<Key, Value> {
    ...
    public final void set(Key key, Value value) {
        ...
        ArraysKt.copyInto(objArr3, destKeys, insertIndex + 1, insertIndex, i);
        if (resize) {
        ...
        ArraysKt.copyInto(this.values, destValues, insertIndex + 1, insertIndex, this.size);
        if (resize) {
    ...
    public final Value remove(Key key) {
        ...
        Value value = (Value) this.values[index];
        int size = this.size;
    ...
    public final void removeIf(Function2<? super Key, ? super Value, Boolean> block) {
        ...
        for (int index = 0; index < size$runtime_release; index++) {
            ...
            Object value = getValues$runtime_release()[index];
        ...
        int index2 = getSize$runtime_release();
        if (index2 > current) {
            int size$runtime_release2 = getSize$runtime_release();
            for (int index3 = current; index3 < size$runtime_release2; index3++) {
                getKeys$runtime_release()[index3] = null;
                getValues$runtime_release()[index3] = null;
            }
    ...
    public final void removeValueIf(Function1<? super Value, Boolean> block) {
        ...
        for (int index$iv = 0; index$iv < size$runtime_release; index$iv++) {
            ...
            Object value$iv = getValues$runtime_release()[index$iv];
        ...
        int index$iv2 = getSize$runtime_release();
        if (index$iv2 <= current$iv) {
            return;
        }
        int size$runtime_release2 = getSize$runtime_release();
        for (int index$iv3 = current$iv; index$iv3 < size$runtime_release2; index$iv3++) {
            getKeys$runtime_release()[index$iv3] = null;
            getValues$runtime_release()[index$iv3] = null;
        }
