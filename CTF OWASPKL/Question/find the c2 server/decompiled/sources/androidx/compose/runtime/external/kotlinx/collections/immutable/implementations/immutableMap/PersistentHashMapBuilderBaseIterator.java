...
public class PersistentHashMapBuilderBaseIterator<K, V, T> extends PersistentHashMapBaseIterator<K, V, T> implements Iterator<T>, KMutableIterator {
    ...
    private final void resetPath(int keyHash, TrieNode<?, ?> trieNode, K k, int pathIndex) {
        int shift = pathIndex * 5;
        if (shift > 30) {
            getPath()[pathIndex].reset(trieNode.getBuffer$runtime_release(), trieNode.getBuffer$runtime_release().length, 0);
            while (!Intrinsics.areEqual(getPath()[pathIndex].currentKey(), k)) {
        ...
        if (trieNode.hasEntryAt$runtime_release(keyPositionMask)) {
            int keyIndex = trieNode.entryKeyIndex$runtime_release(keyPositionMask);
            getPath()[pathIndex].reset(trieNode.getBuffer$runtime_release(), trieNode.entryCount$runtime_release() * 2, keyIndex);
            setPathLastIndex(pathIndex);
            return;
        }
        int nodeIndex = trieNode.nodeIndex$runtime_release(keyPositionMask);
        TrieNode targetNode = trieNode.nodeAtIndex$runtime_release(nodeIndex);
        getPath()[pathIndex].reset(trieNode.getBuffer$runtime_release(), trieNode.entryCount$runtime_release() * 2, nodeIndex);
        resetPath(keyHash, targetNode, k, pathIndex + 1);
    }
