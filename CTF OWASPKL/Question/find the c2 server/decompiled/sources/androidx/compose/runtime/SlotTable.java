...
public final class SlotTable implements CompositionData, Iterable<CompositionGroup>, KMappedMarker {
        ...
        try {
            ...
            try {
                ...
                while (index$iv < size) {
                    ...
                    try {
                        int indexFor = anchor.toIndexFor(writer$iv);
                        boolean z2 = z;
                        try {
                            int $i$f$write = writer$iv.getCurrentGroup();
                            if (indexFor >= $i$f$write) {
    ...
    private static final int verifyWellFormed$validateGroup(Ref.IntRef current, SlotTable this$0, int parent, int parentEnd) {
        ...
        if (parentIndex == parent) {
            int end = SlotTableKt.access$groupSize(this$0.groups, group) + group;
            if (end <= this$0.groupsSize) {
                if (end <= parentEnd) {
                        ...
                        if (dataStart <= dataEnd) {
                            ...
                            if (slotStart <= dataEnd) {
                                ...
                                if (dataEnd - dataStart >= minSlotsNeeded) {
                                    ...
                                    if ((isNode && this$0.slots[SlotTableKt.access$nodeIndex(this$0.groups, group)] == null) ? false : true) {
                                        int nodeCount = 0;
                                        while (current.element < end) {
                                            nodeCount += verifyWellFormed$validateGroup(current, this$0, group, end);
                                        }
                                        ...
                                        if (expectedNodeCount == nodeCount) {
                                            ...
                                            if (expectedSlotCount == actualSlotCount) {
                                                if (SlotTableKt.access$containsAnyMark(this$0.groups, group)) {
                                                    if (!(group <= 0 || SlotTableKt.access$containsMark(this$0.groups, parent))) {
                                                        throw new IllegalStateException(("Expected group " + parent + " to record it contains a mark because " + group + " does").toString());
                                                    }
                                                }
                                                if (isNode) {
                                                    return 1;
        ...
        throw new IllegalStateException(("Invalid parent index detected at " + group + ", expected parent index to be " + parent + " found " + parentIndex).toString());
    }
    ...
    public final String asString() {
        ...
        StringBuilder $this$asString_u24lambda_u2432 = new StringBuilder();
        $this$asString_u24lambda_u2432.append(super.toString());
        $this$asString_u24lambda_u2432.append('\n');
        int groupsSize = this.groupsSize;
        if (groupsSize > 0) {
            int current = 0;
            while (current < groupsSize) {
                current += emitGroup($this$asString_u24lambda_u2432, current, 0);
            }
        } else {
            $this$asString_u24lambda_u2432.append("<EMPTY>");
        }
        String sb = $this$asString_u24lambda_u2432.toString();
