...
public final class LazyStaggeredGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        if (findVisibleItem != null) {
            long it = findVisibleItem.mo626getOffsetnOccac();
            return Integer.valueOf(this.state.isVertical$foundation_release() ? IntOffset.m5241getYimpl(it) : IntOffset.m5240getXimpl(it));
        }
        return null;
    }
    ...
    public float expectedDistanceTo(int index, int targetScrollOffset) {
        ...
        for (int index$iv$iv = 0; index$iv$iv < size; index$iv$iv++) {
            ...
            sum$iv += this.state.isVertical$foundation_release() ? IntSize.m5281getHeightimpl(it.mo627getSizeYbymL2g()) : IntSize.m5282getWidthimpl(it.mo627getSizeYbymL2g());
        }
        int itemSizeSum = sum$iv;
        int averageMainAxisItemSize = itemSizeSum / (visibleItems.size() * this.state.getLaneCount$foundation_release());
        ...
        return ((averageMainAxisItemSize * indexesDiff) + coercedOffset) - getFirstVisibleItemScrollOffset();
    }
