...
public final class BringIntoViewRequestPriorityQueue {
    ...
    public final boolean enqueue(final ContentInViewModifier.Request request) {
        ...
        if (requestBounds == null) {
            ...
            request.getContinuation().resumeWith(Result.m5433constructorimpl(Unit.INSTANCE));
            return false;
        }
    ...
    public final void resumeAndRemoveAll() {
        ...
        if (i <= last) {
            while (true) {
                ...
                Result.Companion companion = Result.Companion;
                this_$iv2.getContent()[i].getContinuation().resumeWith(Result.m5433constructorimpl(unit));
                if (i == last) {
                    break;
                }
                i++;
    ...
    public final void resumeAndRemoveWhile(Function1<? super Rect, Boolean> block) {
        ...
        while (this.requests.isNotEmpty() && block.invoke(((ContentInViewModifier.Request) this.requests.last()).getCurrentBounds().invoke()).booleanValue()) {
            ...
            Unit unit = Unit.INSTANCE;
            Result.Companion companion = Result.Companion;
            ((ContentInViewModifier.Request) mutableVector.removeAt(this_$iv.getSize() - 1)).getContinuation().resumeWith(Result.m5433constructorimpl(unit));
        }
    }
