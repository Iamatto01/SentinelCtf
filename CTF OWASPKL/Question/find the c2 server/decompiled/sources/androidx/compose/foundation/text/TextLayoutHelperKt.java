...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m788canReuse7_7YC6M(TextLayoutResult canReuse, AnnotatedString text, TextStyle style, List<AnnotatedString.Range<Placeholder>> placeholders, int maxLines, boolean softWrap, int overflow, Density density, LayoutDirection layoutDirection, FontFamily.Resolver fontFamilyResolver, long constraints) {
        ...
        if (!canReuse.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() && Intrinsics.areEqual(layoutInput.getText(), text) && layoutInput.getStyle().hasSameLayoutAffectingAttributes(style) && Intrinsics.areEqual(layoutInput.getPlaceholders(), placeholders) && layoutInput.getMaxLines() == maxLines && layoutInput.getSoftWrap() == softWrap && TextOverflow.m5034equalsimpl0(layoutInput.m4606getOverflowgIe3tQ8(), overflow) && Intrinsics.areEqual(layoutInput.getDensity(), density) && layoutInput.getLayoutDirection() == layoutDirection && Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), fontFamilyResolver) && Constraints.m5080getMinWidthimpl(constraints) == Constraints.m5080getMinWidthimpl(layoutInput.m4605getConstraintsmsEJaDk())) {
            if (softWrap || TextOverflow.m5034equalsimpl0(overflow, TextOverflow.Companion.m5042getEllipsisgIe3tQ8())) {
                return Constraints.m5078getMaxWidthimpl(constraints) == Constraints.m5078getMaxWidthimpl(layoutInput.m4605getConstraintsmsEJaDk()) && Constraints.m5077getMaxHeightimpl(constraints) == Constraints.m5077getMaxHeightimpl(layoutInput.m4605getConstraintsmsEJaDk());
            }
            return true;
        }
        return false;
