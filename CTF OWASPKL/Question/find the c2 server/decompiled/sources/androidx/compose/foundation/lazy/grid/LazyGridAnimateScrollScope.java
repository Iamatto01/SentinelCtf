...
public final class LazyGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        if (item != null) {
            if (this.state.isVertical$foundation_release()) {
                m5240getXimpl = IntOffset.m5241getYimpl(item.mo569getOffsetnOccac());
            } else {
                m5240getXimpl = IntOffset.m5240getXimpl(item.mo569getOffsetnOccac());
            }
            return Integer.valueOf(m5240getXimpl);
        }
        return null;
    }
    ...
    public float expectedDistanceTo(int index, int targetScrollOffset) {
        List visibleItems = this.state.getLayoutInfo().getVisibleItemsInfo();
        int slotsPerLine = this.state.getSlotsPerLine$foundation_release();
        int averageLineMainAxisSize = calculateLineAverageMainAxisSize(visibleItems, this.state.isVertical$foundation_release());
    ...
    private final int calculateLineAverageMainAxisSize(final List<? extends LazyGridItemInfo> list, final boolean isVertical) {
        int m5282getWidthimpl;
        Function1 lineOf = new Function1<Integer, Integer>() { // from class: androidx.compose.foundation.lazy.grid.LazyGridAnimateScrollScope$calculateLineAverageMainAxisSize$lineOf$1
            ...
            public final Integer invoke(int it) {
                return Integer.valueOf(isVertical ? list.get(it).getRow() : list.get(it).getColumn());
            }
        };
        int totalLinesMainAxisSize = 0;
        int linesCount = 0;
        int lineStartIndex = 0;
        while (lineStartIndex < list.size()) {
            ...
            if (currentLine == -1) {
                ...
                while (lineEndIndex < list.size() && lineOf.invoke(Integer.valueOf(lineEndIndex)).intValue() == currentLine) {
                    if (isVertical) {
                        m5282getWidthimpl = IntSize.m5281getHeightimpl(list.get(lineEndIndex).mo570getSizeYbymL2g());
                    } else {
                        m5282getWidthimpl = IntSize.m5282getWidthimpl(list.get(lineEndIndex).mo570getSizeYbymL2g());
                    }
                    lineMainAxisSize = Math.max(lineMainAxisSize, m5282getWidthimpl);
                    lineEndIndex++;
                }
