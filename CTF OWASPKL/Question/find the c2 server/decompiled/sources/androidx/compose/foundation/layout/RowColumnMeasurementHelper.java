...
public final class RowColumnMeasurementHelper {
    ...
    private RowColumnMeasurementHelper(LayoutOrientation orientation, Function5<? super Integer, ? super int[], ? super LayoutDirection, ? super Density, ? super int[], Unit> function5, float arrangementSpacing, SizeMode crossAxisSize, CrossAxisAlignment crossAxisAlignment, List<? extends Measurable> list, Placeable[] placeables) {
    ...
    public final RowColumnMeasureHelperResult m437measureWithoutPlacing_EkL_Y(MeasureScope measureScope, long constraints, int startIndex, int endIndex) {
        int targetSpace;
        boolean anyAlignBy;
        int remainder;
        int fixedSpace;
        ...
        int i3 = measureScope.mo295roundToPx0680j_4(this.arrangementSpacing);
        int fixedSpace2 = 0;
        int crossAxisSpace2 = 0;
        int subSize2 = endIndex - startIndex;
        int i4 = startIndex;
        float totalWeight = 0.0f;
        int weightChildrenCount = 0;
        boolean anyAlignBy3 = false;
        ...
        while (i4 < endIndex) {
            ...
            if (weight > 0.0f) {
                ...
                int mainAxisMax2 = constraints2.getMainAxisMax();
                Placeable placeable = this.placeables[i4];
                if (placeable == null) {
                    if (mainAxisMax2 == Integer.MAX_VALUE) {
                        i2 = Integer.MAX_VALUE;
                    } else {
                        i2 = mainAxisMax2 - fixedSpace2;
                    }
                    int i5 = i2;
                    mainAxisMax = mainAxisMax2;
                    i = i4;
                    subSize = subSize2;
                    placeable = child.mo4125measureBRTryo0(OrientationIndependentConstraints.copy$default(constraints2, 0, i5, 0, 0, 8, null).m406toBoxConstraintsOenEA2s(this.orientation));
                } else {
                    mainAxisMax = mainAxisMax2;
                    i = i4;
                    subSize = subSize2;
                ...
                anyAlignBy3 = anyAlignBy3 || RowColumnImplKt.isRelative(parentData);
                this.placeables[i] = placeable2;
                spaceAfterLastNoWeight = spaceAfterLastNoWeight2;
            }
            i4 = i + 1;
            subSize2 = subSize;
        }
        int subSize3 = subSize2;
        int weightedSpace = 0;
        if (weightChildrenCount == 0) {
            anyAlignBy = anyAlignBy3;
            remainder = 0;
            ...
            int remainingToTarget3 = (targetSpace - fixedSpace2) - ((weightChildrenCount - 1) * i3);
            float weightUnitSpace2 = totalWeight > 0.0f ? remainingToTarget3 / totalWeight : 0.0f;
            Iterator<Integer> it = RangesKt.until(startIndex, endIndex).iterator();
            int i6 = 0;
            while (it.hasNext()) {
                i6 += MathKt.roundToInt(RowColumnImplKt.getWeight(this.rowColumnParentData[((IntIterator) it).nextInt()]) * weightUnitSpace2);
            }
            int childMainAxisSize = remainingToTarget3 - i6;
            int i7 = startIndex;
            while (i7 < endIndex) {
                if (this.placeables[i7] != null) {
                    targetSpace2 = targetSpace;
                    remainingToTarget = remainingToTarget3;
                    weightUnitSpace = weightUnitSpace2;
                } else {
                    Measurable child2 = this.measurables.get(i7);
                    RowColumnParentData parentData2 = this.rowColumnParentData[i7];
                    float weight2 = RowColumnImplKt.getWeight(parentData2);
                    if (!(weight2 > 0.0f)) {
                        throw new IllegalStateException("All weights <= 0 should have placeables".toString());
                    }
                    int remainderUnit = MathKt.getSign(childMainAxisSize);
                    int remainder2 = childMainAxisSize - remainderUnit;
                    int remainder3 = MathKt.roundToInt(weightUnitSpace2 * weight2) + remainderUnit;
                    targetSpace2 = targetSpace;
                    int childMainAxisSize2 = Math.max(0, remainder3);
                    if (RowColumnImplKt.getFill(parentData2)) {
                        remainingToTarget = remainingToTarget3;
                        if (childMainAxisSize2 != Integer.MAX_VALUE) {
                            remainingToTarget2 = childMainAxisSize2;
                            weightUnitSpace = weightUnitSpace2;
                            Placeable placeable3 = child2.mo4125measureBRTryo0(new OrientationIndependentConstraints(remainingToTarget2, childMainAxisSize2, 0, constraints2.getCrossAxisMax()).m406toBoxConstraintsOenEA2s(this.orientation));
                            weightedSpace += mainAxisSize(placeable3);
                            crossAxisSpace2 = Math.max(crossAxisSpace2, crossAxisSize(placeable3));
                            anyAlignBy3 = !anyAlignBy3 || RowColumnImplKt.isRelative(parentData2);
                            this.placeables[i7] = placeable3;
                            childMainAxisSize = remainder2;
                        }
                    } else {
                        remainingToTarget = remainingToTarget3;
                    }
                    ...
                    weightUnitSpace = weightUnitSpace2;
                    Placeable placeable32 = child2.mo4125measureBRTryo0(new OrientationIndependentConstraints(remainingToTarget2, childMainAxisSize2, 0, constraints2.getCrossAxisMax()).m406toBoxConstraintsOenEA2s(this.orientation));
                    weightedSpace += mainAxisSize(placeable32);
                    crossAxisSpace2 = Math.max(crossAxisSpace2, crossAxisSize(placeable32));
                    anyAlignBy3 = !anyAlignBy3 || RowColumnImplKt.isRelative(parentData2);
                    this.placeables[i7] = placeable32;
                    childMainAxisSize = remainder2;
                }
                i7++;
            ...
            anyAlignBy = anyAlignBy3;
            remainder = RangesKt.coerceAtMost(((weightChildrenCount - 1) * i3) + weightedSpace, constraints2.getMainAxisMax() - fixedSpace2);
        ...
        if (!anyAlignBy) {
            ...
            int i8 = startIndex;
            while (i8 < endIndex) {
                Placeable placeable4 = this.placeables[i8];
                Intrinsics.checkNotNull(placeable4);
                CrossAxisAlignment crossAxisAlignment = RowColumnImplKt.getCrossAxisAlignment(this.rowColumnParentData[i8]);
        ...
        int beforeCrossAxisAlignmentLine3 = fixedSpace + remainder;
        int mainAxisLayoutSize = Math.max(beforeCrossAxisAlignmentLine3, constraints2.getMainAxisMin());
        if (constraints2.getCrossAxisMax() != Integer.MAX_VALUE && this.crossAxisSize == SizeMode.Expand) {
            crossAxisLayoutSize = constraints2.getCrossAxisMax();
        } else {
            crossAxisLayoutSize = Math.max(crossAxisSpace, Math.max(constraints2.getCrossAxisMin(), beforeCrossAxisAlignmentLine + afterCrossAxisAlignmentLine));
        }
        int[] mainAxisPositions = new int[subSize3];
        for (int i9 = 0; i9 < subSize3; i9++) {
            mainAxisPositions[i9] = 0;
        }
        int[] childrenMainAxisSize = new int[subSize3];
        for (int i10 = 0; i10 < subSize3; i10++) {
            Placeable placeable5 = this.placeables[i10 + startIndex];
            Intrinsics.checkNotNull(placeable5);
            childrenMainAxisSize[i10] = mainAxisSize(placeable5);
        }
