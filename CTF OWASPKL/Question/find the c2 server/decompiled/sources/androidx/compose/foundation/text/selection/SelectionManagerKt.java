...
public final class SelectionManagerKt {
    ...
    private static final long calculateSelectionMagnifierCenterAndroid_O0kMr_c$getMagnifierCenter(SelectionManager $manager, long $magnifierSize, Selection.AnchorInfo anchor, boolean isStartHandle) {
        ...
        if (selectable != null && (containerCoordinates = $manager.getContainerLayoutCoordinates()) != null && (selectableCoordinates = selectable.getLayoutCoordinates()) != null) {
            ...
            Offset m846getCurrentDragPosition_m7T9E = $manager.m846getCurrentDragPosition_m7T9E();
            Intrinsics.checkNotNull(m846getCurrentDragPosition_m7T9E);
            long localDragPosition = selectableCoordinates.mo4131localPositionOfR5De75A(containerCoordinates, m846getCurrentDragPosition_m7T9E.m2378unboximpl());
            float dragX = Offset.m2368getXimpl(localDragPosition);
            long line = selectable.mo807getRangeOfLineContainingjx7JFs(offset);
            Rect lineMin = selectable.getBoundingBox(TextRange.m4635getMinimpl(line));
            Rect lineMax = selectable.getBoundingBox(RangesKt.coerceAtLeast(TextRange.m4634getMaximpl(line) - 1, TextRange.m4635getMinimpl(line)));
            ...
            if (Math.abs(dragX - maxX) > IntSize.m5282getWidthimpl($magnifierSize) / 2) {
                return Offset.Companion.m2383getUnspecifiedF1C5BW0();
            }
            Rect anchorBounds = selectable.getBoundingBox(offset);
            float centerY = Offset.m2369getYimpl(anchorBounds.m2398getCenterF1C5BW0());
            return containerCoordinates.mo4131localPositionOfR5De75A(selectableCoordinates, OffsetKt.Offset(maxX, centerY));
        }
        return Offset.Companion.m2383getUnspecifiedF1C5BW0();
    }

    /* renamed from: calculateSelectionMagnifierCenterAndroid-O0kMr_c  reason: not valid java name */
    public static final long m857calculateSelectionMagnifierCenterAndroidO0kMr_c(SelectionManager manager, long magnifierSize) {
    ...
    public static final Rect visibleBounds(LayoutCoordinates $this$visibleBounds) {
        ...
        return RectKt.m2406Rect0a9Yr6o($this$visibleBounds.mo4135windowToLocalMKHz9U(boundsInWindow.m2403getTopLeftF1C5BW0()), $this$visibleBounds.mo4135windowToLocalMKHz9U(boundsInWindow.m2397getBottomRightF1C5BW0()));
    }

    /* renamed from: containsInclusive-Uv8p0NA  reason: not valid java name */
    public static final boolean m858containsInclusiveUv8p0NA(Rect containsInclusive, long offset) {
        Intrinsics.checkNotNullParameter(containsInclusive, "$this$containsInclusive");
        float left = containsInclusive.getLeft();
        float right = containsInclusive.getRight();
        float m2368getXimpl = Offset.m2368getXimpl(offset);
        if (left <= m2368getXimpl && m2368getXimpl <= right) {
            float top = containsInclusive.getTop();
            float bottom = containsInclusive.getBottom();
            float m2369getYimpl = Offset.m2369getYimpl(offset);
            if (top <= m2369getYimpl && m2369getYimpl <= bottom) {
                return true;
            }
        }
        return false;
