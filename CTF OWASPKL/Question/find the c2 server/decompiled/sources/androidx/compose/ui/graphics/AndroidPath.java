...
public final class AndroidPath implements Path {
    ...
    public int mo2500getFillTypeRgk1Os() {
        if (this.internalPath.getFillType() == Path.FillType.EVEN_ODD) {
            return PathFillType.Companion.m2879getEvenOddRgk1Os();
        }
        return PathFillType.Companion.m2880getNonZeroRgk1Os();
    }

    @Override // androidx.compose.ui.graphics.Path
    /* renamed from: setFillType-oQ8Xj4U  reason: not valid java name */
    public void mo2502setFillTypeoQ8Xj4U(int value) {
        Path.FillType fillType;
        android.graphics.Path path = this.internalPath;
        if (PathFillType.m2875equalsimpl0(value, PathFillType.Companion.m2879getEvenOddRgk1Os())) {
    ...
    public void addRoundRect(RoundRect roundRect) {
        ...
        this.radii[0] = CornerRadius.m2343getXimpl(roundRect.m2418getTopLeftCornerRadiuskKHJgLs());
        this.radii[1] = CornerRadius.m2344getYimpl(roundRect.m2418getTopLeftCornerRadiuskKHJgLs());
        this.radii[2] = CornerRadius.m2343getXimpl(roundRect.m2419getTopRightCornerRadiuskKHJgLs());
        this.radii[3] = CornerRadius.m2344getYimpl(roundRect.m2419getTopRightCornerRadiuskKHJgLs());
        this.radii[4] = CornerRadius.m2343getXimpl(roundRect.m2417getBottomRightCornerRadiuskKHJgLs());
        this.radii[5] = CornerRadius.m2344getYimpl(roundRect.m2417getBottomRightCornerRadiuskKHJgLs());
        this.radii[6] = CornerRadius.m2343getXimpl(roundRect.m2416getBottomLeftCornerRadiuskKHJgLs());
        this.radii[7] = CornerRadius.m2344getYimpl(roundRect.m2416getBottomLeftCornerRadiuskKHJgLs());
        this.internalPath.addRoundRect(this.rectF, this.radii, Path.Direction.CCW);
    }

    @Override // androidx.compose.ui.graphics.Path
    /* renamed from: addPath-Uv8p0NA  reason: not valid java name */
    public void mo2499addPathUv8p0NA(Path path, long offset) {
        Intrinsics.checkNotNullParameter(path, "path");
        android.graphics.Path path2 = this.internalPath;
        if (path instanceof AndroidPath) {
            path2.addPath(((AndroidPath) path).getInternalPath(), Offset.m2368getXimpl(offset), Offset.m2369getYimpl(offset));
            return;
        }
    ...
    public void mo2503translatek4lQ0M(long offset) {
        this.mMatrix.reset();
        this.mMatrix.setTranslate(Offset.m2368getXimpl(offset), Offset.m2369getYimpl(offset));
    ...
    public boolean mo2501opN5in7k0(Path path1, Path path2, int operation) {
        Path.Op op;
        Intrinsics.checkNotNullParameter(path1, "path1");
        Intrinsics.checkNotNullParameter(path2, "path2");
        if (PathOperation.m2884equalsimpl0(operation, PathOperation.Companion.m2888getDifferenceb3I0S0c())) {
            op = Path.Op.DIFFERENCE;
        } else if (PathOperation.m2884equalsimpl0(operation, PathOperation.Companion.m2889getIntersectb3I0S0c())) {
            op = Path.Op.INTERSECT;
        } else if (PathOperation.m2884equalsimpl0(operation, PathOperation.Companion.m2890getReverseDifferenceb3I0S0c())) {
            op = Path.Op.REVERSE_DIFFERENCE;
        } else {
            op = PathOperation.m2884equalsimpl0(operation, PathOperation.Companion.m2891getUnionb3I0S0c()) ? Path.Op.UNION : Path.Op.XOR;
        }
        android.graphics.Path path = this.internalPath;
