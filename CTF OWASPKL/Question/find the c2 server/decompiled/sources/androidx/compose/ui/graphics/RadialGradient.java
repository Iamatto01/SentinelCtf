...
public final class RadialGradient extends ShaderBrush {
    ...
    */
    ...
    public Shader mo2575createShaderuvyYCjk(long size) {
        float centerX;
        float centerY;
        if (OffsetKt.m2389isUnspecifiedk4lQ0M(this.center)) {
            long drawCenter = SizeKt.m2447getCenteruvyYCjk(size);
            centerX = Offset.m2368getXimpl(drawCenter);
            centerY = Offset.m2369getYimpl(drawCenter);
        } else {
            centerX = (Offset.m2368getXimpl(this.center) > Float.POSITIVE_INFINITY ? 1 : (Offset.m2368getXimpl(this.center) == Float.POSITIVE_INFINITY ? 0 : -1)) == 0 ? Size.m2437getWidthimpl(size) : Offset.m2368getXimpl(this.center);
            centerY = (Offset.m2369getYimpl(this.center) > Float.POSITIVE_INFINITY ? 1 : (Offset.m2369getYimpl(this.center) == Float.POSITIVE_INFINITY ? 0 : -1)) == 0 ? Size.m2434getHeightimpl(size) : Offset.m2369getYimpl(this.center);
        }
        ...
        if (f == Float.POSITIVE_INFINITY) {
            f = Size.m2436getMinDimensionimpl(size) / 2;
        }
        return ShaderKt.m2913RadialGradientShader8uybcMk(Offset, f, list, list2, this.tileMode);
    }
    ...
    public boolean equals(Object other) {
        ...
        if ((other instanceof RadialGradient) && Intrinsics.areEqual(this.colors, ((RadialGradient) other).colors) && Intrinsics.areEqual(this.stops, ((RadialGradient) other).stops) && Offset.m2365equalsimpl0(this.center, ((RadialGradient) other).center)) {
            return ((this.radius > ((RadialGradient) other).radius ? 1 : (this.radius == ((RadialGradient) other).radius ? 0 : -1)) == 0) && TileMode.m2965equalsimpl0(this.tileMode, ((RadialGradient) other).tileMode);
        }
        return false;
    }
    ...
    public int hashCode() {
        ...
        return (((((result2 * 31) + Offset.m2370hashCodeimpl(this.center)) * 31) + Float.hashCode(this.radius)) * 31) + TileMode.m2966hashCodeimpl(this.tileMode);
    }

    public String toString() {
        String centerValue = OffsetKt.m2387isSpecifiedk4lQ0M(this.center) ? "center=" + ((Object) Offset.m2376toStringimpl(this.center)) + ", " : "";
        float f = this.radius;
        String radiusValue = !Float.isInfinite(f) && !Float.isNaN(f) ? "radius=" + this.radius + ", " : "";
        return "RadialGradient(colors=" + this.colors + ", stops=" + this.stops + ", " + centerValue + radiusValue + "tileMode=" + ((Object) TileMode.m2967toStringimpl(this.tileMode)) + ')';
    }
}
