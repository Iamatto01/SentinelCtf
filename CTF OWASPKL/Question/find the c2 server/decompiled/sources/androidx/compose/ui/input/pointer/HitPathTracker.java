...
public final class HitPathTracker {
    ...
    public final void m3938addHitPathKNwqfcY(long pointerId, List<? extends PointerInputModifierNode> pointerInputNodes) {
        Node node;
        Node node2;
        ...
        for (int i = 0; i < size; i++) {
            PointerInputModifierNode pointerInputNode = pointerInputNodes.get(i);
            if (merging) {
                MutableVector this_$iv = parent.getChildren();
                int size$iv = this_$iv.getSize();
                if (size$iv > 0) {
                    int i$iv = 0;
                    Node[] content = this_$iv.getContent();
                    do {
                        node2 = content[i$iv];
                        Node it = node2;
                        if (Intrinsics.areEqual(it.getPointerInputNode(), pointerInputNode)) {
                            break;
                        }
                        i$iv++;
                    } while (i$iv < size$iv);
                ...
                if (node != null) {
                    node.markIsIn();
                    if (!node.getPointerIds().contains(PointerId.m3989boximpl(pointerId))) {
                        node.getPointerIds().add(PointerId.m3989boximpl(pointerId));
                    }
