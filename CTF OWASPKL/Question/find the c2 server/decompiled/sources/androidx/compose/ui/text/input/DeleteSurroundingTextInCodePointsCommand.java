    ...
    public void applyTo(EditingBuffer buffer) {
        ...
        for (int i2 = 0; i2 < i; i2++) {
            ...
            if (buffer.getSelectionStart$ui_text_release() > beforeLenInChars) {
                char lead = buffer.get$ui_text_release((buffer.getSelectionStart$ui_text_release() - beforeLenInChars) - 1);
                char trail = buffer.get$ui_text_release(buffer.getSelectionStart$ui_text_release() - beforeLenInChars);
        ...
        for (int i4 = 0; i4 < i3; i4++) {
            ...
            if (buffer.getSelectionEnd$ui_text_release() + afterLenInChars < buffer.getLength$ui_text_release()) {
                char lead2 = buffer.get$ui_text_release((buffer.getSelectionEnd$ui_text_release() + afterLenInChars) - 1);
                char trail2 = buffer.get$ui_text_release(buffer.getSelectionEnd$ui_text_release() + afterLenInChars);
        ...
        int i5 = buffer.getSelectionEnd$ui_text_release();
        buffer.delete$ui_text_release(i5, buffer.getSelectionEnd$ui_text_release() + afterLenInChars);
