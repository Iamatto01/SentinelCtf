...
public final class MotionEventAdapter {
    ...
    private final long m3940getComposePointerId_I2yYro(int motionEventPointerId) {
        long newId;
        ...
        return PointerId.m3990constructorimpl(newId);
    }
    ...
    private final PointerInputEventData createPointerInputEventData(PositionCalculator positionCalculator, MotionEvent motionEvent, int index, boolean pressed) {
        ...
        long pointerId = m3940getComposePointerId_I2yYro(motionEventPointerId);
        ...
        if (index == 0) {
            ...
            long position2 = positionCalculator.mo4088screenToLocalMKHz9U(rawPosition2);
            position = position2;
            rawPosition = rawPosition2;
        } else {
            long rawPosition3 = MotionEventHelper.INSTANCE.m3941toRawOffsetdBAh8RU(motionEvent, index);
            long position3 = positionCalculator.mo4088screenToLocalMKHz9U(rawPosition3);
            position = position3;
            rawPosition = rawPosition3;
        }
        long position4 = motionEvent.getToolType(index);
        switch (position4) {
            case 0:
                toolType = PointerType.Companion.m4086getUnknownT8wyACA();
                break;
            case 1:
                toolType = PointerType.Companion.m4085getTouchT8wyACA();
                break;
            case 2:
                toolType = PointerType.Companion.m4084getStylusT8wyACA();
                break;
            case 3:
                toolType = PointerType.Companion.m4083getMouseT8wyACA();
                break;
            case 4:
                toolType = PointerType.Companion.m4082getEraserT8wyACA();
                break;
            default:
                toolType = PointerType.Companion.m4086getUnknownT8wyACA();
                break;
        ...
        for (int i = 0; i < historySize; i++) {
            ...
            if ((Float.isInfinite(x) || Float.isNaN(x)) ? false : true) {
                if ((Float.isInfinite(y) || Float.isNaN(y)) ? false : false) {
        ...
        if (motionEvent.getActionMasked() == 8) {
            ...
            scrollDelta = Offset.Companion.m2384getZeroF1C5BW0();
        }
