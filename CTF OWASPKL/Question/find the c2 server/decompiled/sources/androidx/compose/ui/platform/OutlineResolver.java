...
public final class OutlineResolver {
    ...
    public OutlineResolver(Density density) {
        ...
        this.size = Size.Companion.m2446getZeroNHjbRc();
        this.shape = RectangleShapeKt.getRectangleShape();
        this.rectTopLeft = Offset.Companion.m2384getZeroF1C5BW0();
        this.rectSize = Size.Companion.m2446getZeroNHjbRc();
    ...
    public final boolean m4455isInOutlinek4lQ0M(long position) {
        androidx.compose.ui.graphics.Outline outline;
        if (this.outlineNeeded && (outline = this.calculatedOutline) != null) {
            return ShapeContainingUtilKt.isInOutline(outline, Offset.m2368getXimpl(position), Offset.m2369getYimpl(position), this.tmpTouchPointPath, this.tmpOpPath);
        }
        return true;
    }
    ...
    public final void clipToOutline(Canvas canvas) {
        ...
        if (targetPath != null) {
            Canvas.m2577clipPathmtrdDE$default(canvas, targetPath, 0, 2, null);
            return;
        }
        ...
        if (f > 0.0f) {
            ...
            if (roundRectClipPath == null || !m4454isSameBounds4L21HEs(roundRect, this.rectTopLeft, this.rectSize, f)) {
                RoundRect roundRect2 = RoundRectKt.m2422RoundRectgG7oq9Y(Offset.m2368getXimpl(this.rectTopLeft), Offset.m2369getYimpl(this.rectTopLeft), Offset.m2368getXimpl(this.rectTopLeft) + Size.m2437getWidthimpl(this.rectSize), Offset.m2369getYimpl(this.rectTopLeft) + Size.m2434getHeightimpl(this.rectSize), CornerRadiusKt.CornerRadius$default(this.roundedCornerRadius, 0.0f, 2, null));
            ...
            Canvas.m2577clipPathmtrdDE$default(canvas, roundRectClipPath, 0, 2, null);
            return;
        }
        Canvas.m2578clipRectN_I0leg$default(canvas, Offset.m2368getXimpl(this.rectTopLeft), Offset.m2369getYimpl(this.rectTopLeft), Offset.m2368getXimpl(this.rectTopLeft) + Size.m2437getWidthimpl(this.rectSize), Offset.m2369getYimpl(this.rectTopLeft) + Size.m2434getHeightimpl(this.rectSize), 0, 16, null);
    }

    /* renamed from: update-uvyYCjk  reason: not valid java name */
    public final void m4456updateuvyYCjk(long size) {
        if (!Size.m2433equalsimpl0(this.size, size)) {
            this.size = size;
            this.cacheIsDirty = true;
        }
    }
    ...
    private final void updateCache() {
        if (this.cacheIsDirty) {
            ...
            if (this.outlineNeeded && Size.m2437getWidthimpl(j) > 0.0f && Size.m2434getHeightimpl(this.size) > 0.0f) {
                this.isSupportedOutline = true;
                androidx.compose.ui.graphics.Outline outline = this.shape.mo193createOutlinePq9zytI(this.size, this.layoutDirection, this.density);
    ...
    private final void updateCacheWithRoundRect(RoundRect roundRect) {
        float radius = CornerRadius.m2343getXimpl(roundRect.m2418getTopLeftCornerRadiuskKHJgLs());
    ...
    private final boolean m4454isSameBounds4L21HEs(RoundRect $this$isSameBounds_u2d4L21HEs, long offset, long size, float radius) {
        if ($this$isSameBounds_u2d4L21HEs == null || !RoundRectKt.isSimple($this$isSameBounds_u2d4L21HEs)) {
            return false;
        }
        if (!($this$isSameBounds_u2d4L21HEs.getLeft() == Offset.m2368getXimpl(offset))) {
            return false;
        }
        if (!($this$isSameBounds_u2d4L21HEs.getTop() == Offset.m2369getYimpl(offset))) {
            return false;
        }
        if (!($this$isSameBounds_u2d4L21HEs.getRight() == Offset.m2368getXimpl(offset) + Size.m2437getWidthimpl(size))) {
            return false;
        }
        if ($this$isSameBounds_u2d4L21HEs.getBottom() == Offset.m2369getYimpl(offset) + Size.m2434getHeightimpl(size)) {
            return (CornerRadius.m2343getXimpl($this$isSameBounds_u2d4L21HEs.m2418getTopLeftCornerRadiuskKHJgLs()) > radius ? 1 : (CornerRadius.m2343getXimpl($this$isSameBounds_u2d4L21HEs.m2418getTopLeftCornerRadiuskKHJgLs()) == radius ? 0 : -1)) == 0;
        }
        return false;
    }
}
