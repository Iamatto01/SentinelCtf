...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m6894plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m6805toLongimpl(duration, getUnit());
        if (longDelta != Long.MIN_VALUE && longDelta != Long.MAX_VALUE) {
            long j = this.reading;
            newReading = j + longDelta;
            if ((j ^ longDelta) >= 0 && (j ^ newReading) < 0) {
                m6893overflowLRDsOJo(duration);
            }
        } else {
            double delta = Duration.m6802toDoubleimpl(duration, getUnit());
    ...
    private final void m6893overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m6808toStringimpl(duration)) + '.');
    }
}
