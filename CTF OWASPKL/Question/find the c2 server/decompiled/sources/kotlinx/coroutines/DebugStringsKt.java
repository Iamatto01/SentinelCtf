...
public final class DebugStringsKt {
    ...
    public static final String toDebugString(Continuation<?> continuation) {
        String m5433constructorimpl;
        if (continuation instanceof DispatchedContinuation) {
            return continuation.toString();
        }
        try {
            Result.Companion companion = Result.Companion;
            m5433constructorimpl = Result.m5433constructorimpl(continuation + '@' + getHexAddress(continuation));
        } catch (Throwable th) {
            Result.Companion companion2 = Result.Companion;
            m5433constructorimpl = Result.m5433constructorimpl(ResultKt.createFailure(th));
        }
        if (Result.m5436exceptionOrNullimpl(m5433constructorimpl) != null) {
            m5433constructorimpl = continuation.getClass().getName() + '@' + getHexAddress(continuation);
        }
        return (String) m5433constructorimpl;
    }
